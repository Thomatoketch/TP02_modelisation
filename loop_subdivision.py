...
def loop_subdivision(mesh: Mesh, iterations: int = 1) -> Mesh:
    for _ in range(iterations):
        ...
        for he in mesh.half_edges:
            ...
            if he.twin:
                v3 = he.next.target_vertex
                v4 = he.twin.next.target_vertex
                pos = (
                    3/8 * np.array([v1.x, v1.y, v1.z]) +
                    3/8 * np.array([v2.x, v2.y, v2.z]) +
                    1/8 * np.array([v3.x, v3.y, v3.z]) +
                    1/8 * np.array([v4.x, v4.y, v4.z])
                )
            else:
        ...
        for v in mesh.vertex:
            ...
            he = he_start
            while True:
                neighbors.append(he.target_vertex)
                he = he.twin.next if he.twin else None
                if he is None or he == he_start:
                    break
            ...
            neighbor_sum = sum(np.array([v_i.x, v_i.y, v_i.z]) for v_i in neighbors)
            new_pos = (1 - n * beta) * np.array([v.x, v.y, v.z]) + beta * neighbor_sum
            v.x, v.y, v.z = new_pos
        ...
        for face in mesh.faces:
            ...
            def get_mid_vertex(a, b):
                ...
                if key not in edge_vertex_map:
                    raise ValueError("Midpoint not found.")
